  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 w-full overflow-hidden">
      ...
      <div className="flex gap-6 w-full">
        ...
        <main className="flex-1 min-w-0 pb-20 lg:pb-0 overflow-hidden">
          {/* Section header */}
          <motion.div
          ...
          >
            <span className="grid place-items-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-primary/10 text-primary flex-shrink-0">
              {(() => {
...
function AdminRooms() {
  ...
  const [form, setForm] = useState({
    ...
    tasksRequired: 0, isHidden: false,
    startTime: "", endTime: "",
  });
  ...
  const submit = (e: React.FormEvent) => {
    ...
    addRoom({
      ...
      rewardPoints: form.rewardPoints,
      tasksRequired: form.tasksRequired, isHidden: form.isHidden,
      tasks: [], startTime: new Date(form.startTime).toISOString(),
      endTime: new Date(form.endTime).toISOString(), status: "open",
    } as any);
    toast.success("Room created");
    setOpen(false);
    setForm({ name: "", description: "", level: 1, entryPoints: 0, entryCost: 0, rewardPoints: 100, tasksRequired: 0, isHidden: false, startTime: "", endTime: "" });
  };
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {(r as any).isHidden && <Badge variant="destructive" className="text-xs"><Lock className="w-2.5 h-2.5 mr-1" />Locked</Badge>}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                ...
                <div><span className="text-muted-foreground">Tasks req: </span>{(r as any).tasksRequired || 0}</div>
              </div>
              {(r as any).tasksRequired > 0 && (
                <div className="mt-2 p-2 rounded bg-orange-50 text-orange-700 text-xs flex items-center gap-1">
                  <Lock className="w-3 h-3" />
                  Users must complete {(r as any).tasksRequired} tasks to join
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="rtasks">Tasks Required (0 = no lock)</Label>
                <Input id="rtasks" type="number" min={0} required value={form.tasksRequired} onChange={(e) => setForm({ ...form, tasksRequired: parseInt(e.target.value) || 0 })} placeholder="0" />
              </div>
              <div className="space-y-2 flex items-end">
                <label className="flex items-center gap-2 cursor-pointer pb-2">
                  <input type="checkbox" checked={form.isHidden} onChange={(e) => setForm({ ...form, isHidden: e.target.checked })} className="w-4 h-4 rounded" />
                  <span className="text-sm">Hide room until unlocked</span>
                </label>
              </div>
            </div>
            <div className="rounded-lg bg-muted/50 p-2 text-xs text-muted-foreground">
              <Lock className="w-3 h-3 inline mr-1" />
              If Tasks Required &gt; 0, users must complete that many tasks before they can join this room.
            </div>
            <div className="grid grid-cols-2 gap-3">
        ...
        <CardContent className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold">Official Referral Links</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Generate official referral links for your team leads, partners, and promoters. Each link has no referrer attached — it&apos;s a root official link. Share these links with your audience so they can register through your official channels.
              </p>
            </div>
            <Button onClick={() => setOpen(true)}><Plus className="w-4 h-4 mr-2" /> Generate Link</Button>
          </div>
        </CardContent>
      </Card>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge variant="secondary" className="bg-primary/10 text-primary">{link.referralCode}</Badge>
                        <Badge variant="outline">@{link.username}</Badge>
                        <Badge variant="outline">{registrations} registrations</Badge>
                      </div>
                      <p className="font-medium text-sm">{link.label}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <code className="text-xs bg-muted px-2 py-1 rounded flex-1 truncate">{link.referralLink}</code>
                ...
                {businesses.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="p-3 text-right">{bizCampaigns.length}</td>
                      <td className="p-3 text-right font-medium">{formatUSD(budget)}</td>
                      <td className="p-3 text-right text-red-600">-{formatUSD(spent)}</td>
                      <td className="p-3 text-right">{formatPoints(views)}</td>
                      <td className="p-3 text-right">{formatPoints(clicks)}</td>
                    </tr>
                  );
                })}
                {businesses.length === 0 && (
                  <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">No businesses registered yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function AdminQuizzes() {
  const { quizzes, quizAttempts, addQuiz, deleteQuiz } = useStore();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({
    title: "",
    description: "",
  ...
  const addQuestion = () => {
    setQuestions([...questions, { question: "", options: ["", "", "", ""], correctIndex: 0, points: 1 }]);
  };
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setOpen(false);
    setForm({ title: "", description: "", category: "", rewardPoints: 20, passScore: 60, timeLimitMin: 5 });
    setQuestions([{ question: "", options: ["", "", "", ""], correctIndex: 0, points: 1 }]);
  };

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">{quizzes.length} quizzes • {quizAttempts.length} total attempts</p>
      ...
      {quizzes.length === 0 ? (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Create your first quiz to engage users.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {quizzes.map((q: any) => {
            const attempts = quizAttempts.filter((a: any) => a.quizId === q.id);
            const passedCount = attempts.filter((a: any) => a.passed).length;
            return (
              <Card key={q.id}>
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <Badge variant="outline">{q.category || "General"}</Badge>
                    <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => { deleteQuiz(q.id); toast.success("Quiz deleted"); }}>
                      <Trash2 className="w-3.5 h-3.5 text-red-600" />
                    </Button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                    <div><span className="text-muted-foreground">Questions:</span> {q.questions?.length || 0}</div>
                    <div><span className="text-muted-foreground">Reward:</span> +{q.rewardPoints}</div>
                    <div><span className="text-muted-foreground">Pass:</span> {q.passScore}%</div>
                    <div><span className="text-muted-foreground">Time:</span> {q.timeLimitMin}min</div>
                    <div><span className="text-muted-foreground">Attempts:</span> {attempts.length}</div>
                    <div><span className="text-muted-foreground">Passed:</span> {passedCount}</div>
                  </div>
                </CardContent>
              </Card>
            );
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Add multiple-choice questions. Users earn points based on their score.</DialogDescription>
          </DialogHeader>
          <form onSubmit={submit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2 col-span-2">
                <Label htmlFor="qtitle">Quiz Title</Label>
                <Input id="qtitle" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. General Knowledge Quiz" />
              </div>
              <div className="space-y-2 col-span-2">
                <Label htmlFor="qdesc">Description</Label>
                <Input id="qdesc" required value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="What is this quiz about?" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="qcat">Category (optional)</Label>
                <Input id="qcat" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="e.g. Science, Sports" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="qrew">Reward Points</Label>
                <Input id="qrew" type="number" min={1} required value={form.rewardPoints} onChange={(e) => setForm({ ...form, rewardPoints: parseInt(e.target.value) || 0 })} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="qpass">Pass Score (%)</Label>
                <Input id="qpass" type="number" min={1} max={100} required value={form.passScore} onChange={(e) => setForm({ ...form, passScore: parseInt(e.target.value) || 0 })} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="qtime">Time Limit (min)</Label>
                <Input id="qtime" type="number" min={1} required value={form.timeLimitMin} onChange={(e) => setForm({ ...form, timeLimitMin: parseInt(e.target.value) || 0 })} />
              </div>
            </div>
                      ...
                      {q.options.map((opt, optIdx) => (
                        <div key={optIdx} className="flex items-center gap-2">
                          <button
                            ...
                            className={`w-6 h-6 rounded-full border-2 flex-shrink-0 grid place-items-center text-xs font-bold ${
            ...
            <DialogFooter>
              <Button type="submit" size="lg" className="w-full">Create Quiz</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
