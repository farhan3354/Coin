...
const adminSections: AdminSection[] = [
  ...
  { value: "quizzes", label: "Quizzes", icon: Brain, group: "management" },
    ...
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1">
        <nav className="p-3 space-y-5">
          {(Object.keys(groupLabels) as AdminSection["group"][]).map(
            (group) => {
...
export function AdminDashboard() {
  ...
  const activeLabel =
    adminSections.find((s) => s.value === tab)?.label || "Overview";
        ...
        <main className="flex-1 min-w-0">
          ...
          >
            <span className="grid place-items-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-primary/10 text-primary">
              {(() => {
                const ActiveIcon =
                  adminSections.find((s) => s.value === tab)?.icon ||
                  LayoutDashboard;
...
function AdminOverview() {
  ...
  const pendingW = withdrawals.filter((w) =>
    ["pending", "under-review", "approved"].includes(w.status),
  );
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Total Users"
          value={formatPoints(users.length)}
        ...
        <StatCard
          title="Total Points"
          value={formatPoints(totalPointsCirculating)}
        ...
        <StatCard
          ...
          subtitle={`${pendingW.length} pending`}
          icon={Wallet}
          accent="bg-green-100 text-green-700"
        />
        <StatCard
          title="Ad Revenue"
          value={`$${totalCampaignSpend.toFixed(2)}`}
          subtitle="from businesses"
          icon={Building2}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Videos"
          value={videos.length}
          subtitle={`${videos.filter((v) => v.status === "active").length} active`}
          icon={VideoIcon}
        ...
        <StatCard
          title="Tasks"
          value={tasks.length}
          subtitle={`${tasks.reduce((s, t) => s + t.completed, 0)} completions`}
          icon={Activity}
        ...
        <StatCard
          title="Events"
          value={events.length}
          subtitle={`${events.filter((e) => e.status === "live").length} live`}
          icon={Trophy}
          accent="bg-amber-100 text-amber-700"
        />
        <StatCard
          title="Rooms"
          value={rooms.length}
          subtitle={`${rooms.filter((r) => r.status === "open").length} open`}
          icon={Crown}
          accent="bg-purple-100 text-purple-700"
        />
      </div>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatUSD(w.amountUSD)} · {w.method}
                  </p>
                </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Paste a video URL. We&apos;ll embed it directly on EarnCoin.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select
                  value={form.platform}
                  onValueChange={(v) =>
                    setForm({ ...form, platform: v as VideoPlatform })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="vrew">Reward Points</Label>
                <Input
                  id="vrew"
                  type="number"
                  min={1}
              ...
              <div className="space-y-2">
                <Label htmlFor="vdur">Watch Duration (sec)</Label>
                <Input
                  id="vdur"
                  type="number"
                  min={1}
                  required
                  value={form.watchDurationSec}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      watchDurationSec: parseInt(e.target.value) || 0,
                    })
                  }
                />
              </div>
        ...
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="trew">Reward</Label>
                <Input
                  id="trew"
                  type="number"
                  min={1}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="tav">Availability</Label>
                <Input
                  id="tav"
                  type="number"
                  min={1}
              ...
              <div className="space-y-2">
                <Label htmlFor="tlink">Link</Label>
                <Input
                  id="tlink"
                  value={form.link}
                ...
                {filtered.map((w) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-right">
                      <p className="font-bold">{formatUSD(w.amountUSD)}</p>
                      <p className="text-xs text-muted-foreground">
                    ...
                    <td className="p-3 hidden md:table-cell text-xs text-muted-foreground font-mono truncate max-w-[150px]">
                      {w.accountDetails}
                    </td>
                    <td className="p-3 text-center">
                          ...
                          w.status !== "cancelled" && (
                            <>
                              ...
                              <Button
                                ...
                                title="Hold"
                                onClick={() => {
                                  updateWithdrawalStatus(w.id, "hold");
...
function AdminEvents() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!form.startTime || !form.endTime)
      return toast.error("Pick start and end times");
    addEvent({
      title: form.title,
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {events.map((e) => (
          <Card key={e.id}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <Badge className={`capitalize ${typeColors[e.type]}`}>
                  {e.type}
                </Badge>
                <Button
                  size="icon"
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select
                  value={form.type}
                  onValueChange={(v) =>
                    setForm({ ...form, type: v as EventItem["type"] })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                  ...
                  <SelectContent>
                    {["daily", "weekly", "monthly", "special", "festival"].map(
                      (t) => (
                        <SelectItem key={t} value={t} className="capitalize">
                          {t}
                        </SelectItem>
              ...
              <div className="space-y-2">
                <Label htmlFor="erew">Reward Points</Label>
                <Input
                  id="erew"
                  type="number"
                  min={1}
            ...
            <div className="space-y-2">
              <Label htmlFor="erules">Rules (one per line)</Label>
              <Textarea
                id="erules"
                rows={3}
                value={form.rules}
...
function AdminRooms() {
  ...
  const seatsPerLevel: Record<number, number> = {
    1: 5,
    2: 7,
    3: 10,
    4: 15,
        ...
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 gap-2">
            {[1, 2, 3, 4, 5].map((lvl) => (
              <div
                key={lvl}
                className={`p-3 rounded-lg border text-center ${levelColors[lvl]}`}
              >
                <p className="font-bold">Level {lvl}</p>
                <p className="text-xs">{levelNames[lvl]}</p>
                <p className="text-sm font-semibold mt-1">
                  {seatsPerLevel[lvl]} seats
                </p>
              </div>
            ))}
          </div>
          ...
          <Card key={r.id}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  <div
                    className={`grid place-items-center w-8 h-8 rounded-lg font-bold text-sm ${levelColors[r.level]}`}
                  >
                    L{r.level}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                ...
                <div>
                  <span className="text-muted-foreground">XP req: </span>
                  {formatPoints(r.entryPoints)}
                </div>
                <div>
                  <span className="text-muted-foreground">Cost: </span>
                  {r.entryCost > 0 ? `${r.entryCost} pts` : "Free"}
                </div>
                <div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setForm({
                    ...form,
                    level: parseInt(v) as 1 | 2 | 3 | 4 | 5,
                    entryPoints: [0, 500, 2000, 5000, 10000][parseInt(v) - 1],
                  })
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {[1, 2, 3, 4, 5].map((lvl) => (
                    <SelectItem key={lvl} value={String(lvl)}>
                      Level {lvl} — {levelNames[lvl]} ({seatsPerLevel[lvl]}{" "}
                      seats)
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="space-y-2">
                <Label htmlFor="rep">Min XP</Label>
                <Input
                  id="rep"
                  type="number"
                  min={0}
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      entryPoints: parseInt(e.target.value) || 0,
                    })
                  }
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="rec">Entry Cost</Label>
                <Input
                  id="rec"
                  type="number"
                  min={0}
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      entryCost: parseInt(e.target.value) || 0,
                    })
                  }
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="rrew">Reward</Label>
                <Input
                  id="rrew"
                  type="number"
                  min={1}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="rstart">Start Time</Label>
                <Input
                  id="rstart"
                  type="datetime-local"
                  required
              ...
              <div className="space-y-2">
                <Label htmlFor="rend">End Time</Label>
                <Input
                  id="rend"
                  type="datetime-local"
                  required
...
function AdminOfficialLinks() {
  const { officialLinks, createOfficialLink, deleteOfficialLink, users } =
    useStore();
  const [open, setOpen] = useState(false);
          ...
          <div className="flex flex-col sm:flex-row items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                Generate official referral links for your team leads, partners,
                and promoters. Each link has no referrer attached — it&apos;s a
                root official link.
              </p>
            </div>
            <Button
      ...
      {officialLinks.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            No official links yet. Create one to start.
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mt-2">
                        <code className="text-xs bg-muted px-2 py-1.5 rounded flex-1 truncate block overflow-x-auto">
                          {link.referralLink}
                        </code>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => copyLink(link.referralLink)}
                          >
                            <Copy className="w-3.5 h-3.5" />
                          </Button>
                          <Button size="sm" variant="ghost" asChild>
                            <a
                              href={link.referralLink}
                              target="_blank"
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create an official user link. This will generate a unique referral
              code with no referrer attached — share it with your team and
              audience.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submit} className="space-y-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g. YouTube Channel Partnership"
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g. official_yt"
              />
              <p className="text-xs text-muted-foreground">
                This username will be associated with the referral link. No
                login is needed for this account — it&apos;s just a tracking
                identity.
              </p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
              ...
              <ol className="list-decimal list-inside space-y-0.5">
                ...
                <li>
                  A unique referral code is created (no referrer attached)
                </li>
                <li>Share the link with your team/audience</li>
                <li>
                  They register through the link and become part of your network
                </li>
                <li>You track registrations from the Admin Panel</li>
              </ol>
            </div>
            <DialogFooter>
              <Button type="submit">Generate Official Link</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
...
function AdminGames() {
  ...
  const totalPaidOut = coinGames
    .filter((g) => g.result === "win")
    .reduce((s, g) => s + g.pointsChange, 0);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          title="Total Wagered"
          value={formatPoints(totalWagered)}
          subtitle="coins"
          icon={Coins}
          accent="bg-amber-100 text-amber-700"
        />
        <StatCard
          ...
          value={formatPoints(totalPaidOut)}
          subtitle="to winners"
          icon={TrendingUp}
          accent="bg-green-100 text-green-700"
        />
        <StatCard
          title="House Profit"
          value={formatPoints(houseProfit)}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Game History (All Users)</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
                ...
                <tbody>
                  {gameResults.slice(0, 30).map((g) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">
                        {g.entryFee > 0 ? formatPoints(g.entryFee) : "—"}
                      </td>
                      <td className="p-3 text-center">
                        <Badge
                          variant={g.result === "win" ? "secondary" : "outline"}
                          className={
                            g.result === "win"
                              ? "bg-green-100 text-green-700"
                              : g.result === "loss"
                      ...
                      <td
                        className={`p-3 text-right font-medium ${g.pointsChange > 0 ? "text-green-600" : g.pointsChange < 0 ? "text-red-600" : ""}`}
                      >
                        {g.pointsChange > 0 ? "+" : ""}
                        {g.pointsChange !== 0
...
function AdminAnalytics() {
  const { users, withdrawals, coinHistory, campaigns, events, rooms } =
    useStore();
  ...
  const dailyData = Array.from({ length: 7 }).map((_, i) => ({
    day: new Date(Date.now() - (6 - i) * 86400000).toLocaleDateString("en-US", {
      weekday: "short",
    }),
  ...
  const totalRevenue =
    campaigns.reduce((s, c) => s + c.spent, 0) + totalWithdrawn * 0.1; // mock ad rev share
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={formatPoints(dailyData[6].value)}
        ...
        <StatCard
          title="Ad Revenue"
          value={formatUSD(campaigns.reduce((s, c) => s + c.spent, 0))}
          subtitle="from businesses"
          icon={Building2}
        />
        <StatCard
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-primary" /> Daily Earnings (7
              days)
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Smartphone className="w-5 h-5 text-primary" /> Device / Browser
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Activity className="w-5 h-5 text-primary" /> Referral Statistics
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">
                {realUsers.reduce((s, u) => s + u.totalReferrals, 0)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Active referrals</span>
              <span className="font-medium">
                {realUsers.reduce((s, u) => s + u.activeReferrals, 0)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Event & Room Reports</CardTitle>
        </CardHeader>
        <CardContent className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-3 rounded-lg bg-muted/50">
            <p className="text-xs text-muted-foreground">Open rooms</p>
            <p className="text-xl font-bold">
          ...
          <div className="p-3 rounded-lg bg-muted/50">
            ...
            <p className="text-xl font-bold">
              {events.reduce((s, e) => s + e.participants.length, 0)}
            </p>
          </div>
          <div className="p-3 rounded-lg bg-muted/50">
            <p className="text-xs text-muted-foreground">Room participants</p>
            <p className="text-xl font-bold">
              {rooms.reduce((s, r) => s + r.participants.length, 0)}
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  const exports = [
    {
      label: "Users Report",
      icon: Users,
      fn: () => toast.success("Users PDF exported"),
    },
    {
      label: "Withdrawals Report",
      icon: Wallet,
      fn: () => toast.success("Withdrawals Excel exported"),
    },
    {
      label: "Coin History Report",
      icon: Coins,
      fn: () => toast.success("Coin history CSV exported"),
    },
    {
      label: "Referral Statistics",
      icon: Users,
      fn: () => toast.success("Referral PDF exported"),
    },
    {
      label: "Event Reports",
      icon: Trophy,
      fn: () => toast.success("Event Excel exported"),
    },
    {
      label: "Room Reports",
      icon: Crown,
      fn: () => toast.success("Room CSV exported"),
    },
    {
      label: "Business Revenue",
      icon: Building2,
      fn: () => toast.success("Business PDF exported"),
    },
    {
      label: "Ad Revenue Report",
      icon: TrendingUp,
      fn: () => toast.success("Ad revenue Excel exported"),
    },
  ];
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Download platform reports as PDF, Excel or CSV.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {exports.map((e) => (
            <button
              key={e.label}
              onClick={e.fn}
              className="text-left p-4 rounded-lg border hover:bg-muted/30 transition-colors"
            >
              <e.icon className="w-6 h-6 text-primary mb-2" />
              <p className="font-medium text-sm">{e.label}</p>
              <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                <FileDown className="w-3 h-3" /> PDF / Excel / CSV
              </p>
            </button>
          ))}
        </CardContent>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold">{coinHistory.length}</p>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {businesses.map((b) => {
                  const bizCampaigns = campaigns.filter(
                    (c) => c.businessId === b.id,
                  );
                ...
                {businesses.length === 0 && (
                  <tr>
                    ...
                    <td
                      colSpan={6}
                      className="p-8 text-center text-muted-foreground hidden sm:table-cell"
                    >
                      No businesses registered yet.
                    </td>
                  </tr>
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Control how many points users earn for each activity.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="welcome">Welcome Bonus (points)</Label>
            <Input
              id="welcome"
              type="number"
          ...
          <div className="space-y-2">
            <Label htmlFor="ref">Referral Reward (points)</Label>
            <Input
              id="ref"
              type="number"
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  referralReward: parseInt(e.target.value) || 0,
                })
              }
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="vid">Video Default Reward</Label>
            <Input
              id="vid"
              type="number"
              value={form.videoDefaultReward}
              onChange={(e) =>
                setForm({
                  ...form,
                  videoDefaultReward: parseInt(e.target.value) || 0,
                })
              }
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="task">Task Default Reward</Label>
            <Input
              id="task"
              type="number"
              value={form.taskDefaultReward}
              onChange={(e) =>
                setForm({
                  ...form,
                  taskDefaultReward: parseInt(e.target.value) || 0,
                })
              }
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="evt">Event Default Reward</Label>
            <Input
              id="evt"
              type="number"
              value={form.eventDefaultReward}
              onChange={(e) =>
                setForm({
                  ...form,
                  eventDefaultReward: parseInt(e.target.value) || 0,
                })
              }
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="room">Room Default Reward</Label>
            <Input
              id="room"
              type="number"
              value={form.roomDefaultReward}
              onChange={(e) =>
                setForm({
                  ...form,
                  roomDefaultReward: parseInt(e.target.value) || 0,
                })
              }
            />
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Conversion & Withdrawal</CardTitle>
        </CardHeader>
        <CardContent className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  pointsPerDollar: parseInt(e.target.value) || 1,
                })
              }
            />
            <p className="text-xs text-muted-foreground">
              {formatPoints(form.pointsPerDollar)} points = $1 USD
            </p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="minw">Min Withdrawal ($)</Label>
            <Input
              id="minw"
              type="number"
              step="0.01"
          ...
          <div className="space-y-2">
            <Label htmlFor="maxw">Max Withdrawal ($)</Label>
            <Input
              id="maxw"
              type="number"
              step="0.01"
          ...
          <div className="space-y-2">
            <Label htmlFor="proc">Processing Time</Label>
            <Input
              id="proc"
              value={form.withdrawalProcessingHours}
              onChange={(e) =>
                setForm({ ...form, withdrawalProcessingHours: e.target.value })
              }
            />
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Toggle which methods users can choose.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap gap-2">
          {methods.map((m) => (
            ...
            >
              {form.withdrawalMethods.includes(m) ? (
      ...
      <Card className="border-destructive">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground mb-3">
            Reset all platform data back to seed state. This will log everyone
            out.
          </p>
      ...
      <Dialog open={resetOpen} onOpenChange={setResetOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will wipe all users, withdrawals, videos, tasks, events and
              rooms and restore seed data. This cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                resetData();
                setResetOpen(false);
                toast.success("Platform reset to seed data");
              }}
            >
              Reset
            </Button>
...
function AdminEmails() {
  ...
  const selected = selectedEmail
    ? (emailLogs || []).find((e: any) => e.id === selectedEmail)
    : null;
          ...
          {filtered.length === 0 ? (
            <div className="py-16 text-center text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Emails are automatically sent on registration, OTP verification,
                ...
                <tbody>
                  {filtered.slice(0, 50).map((e: any) => (
                    ...
                    >
                      ...
                      <td className="p-3 hidden sm:table-cell">
                        <Badge
                          className={`capitalize text-xs ${typeColors[e.type] || ""}`}
                        >
                          {e.type.replace("-", " ")}
                        </Badge>
                      </td>
      ...
      <Dialog open={openBroadcast} onOpenChange={setOpenBroadcast}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Send an announcement email to all {subscribedCount} subscribed
              users.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={sendBroadcast} className="space-y-3">
            ...
            <DialogFooter>
              ...
              <Button type="submit">
                <Send className="w-4 h-4 mr-2" /> Send to {subscribedCount}{" "}
                users
              </Button>
            </DialogFooter>
          </form>
      ...
      >
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              To: {selected?.toName} &lt;{selected?.to}&gt; ·{" "}
