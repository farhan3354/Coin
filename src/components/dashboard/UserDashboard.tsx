...
export function UserDashboard() {
  ...
  const now = new Date();
  const today = myHistory.filter(
    (h) => new Date(h.date).toDateString() === now.toDateString(),
  );
  const weekAgo = new Date(now.getTime() - 7 * 86400000);
  const monthAgo = new Date(now.getTime() - 30 * 86400000);
  const todayEarned = today.reduce(
    (s, h) => s + h.pointsEarned - h.pointsDeducted,
    0,
  );
  const weekEarned = myHistory
    .filter((h) => new Date(h.date) >= weekAgo)
    .reduce((s, h) => s + h.pointsEarned, 0);
  const monthEarned = myHistory
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          subtitle={`${formatUSD(user.dollarBalance)} USD`}
          icon={Coins}
        ...
        <StatCard
          title="This Month"
          value={`+${formatPoints(monthEarned)}`}
          subtitle="points earned"
          icon={Calendar}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Invite friends — earn {settings.referralReward} points for each
              verified referral.
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <div className="flex-1 min-w-0 px-3 py-2 rounded-md bg-muted font-mono font-bold tracking-wider truncate">
                    {user.referralCode}
                  </div>
                  <Button
                    size="icon"
                    variant="outline"
                    onClick={() => copyReferral(user.referralCode, "Code")}
                  >
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <div className="flex-1 min-w-0 px-3 py-2 rounded-md bg-muted text-sm truncate">
                    {referralLink}
                  </div>
                  <Button
                    size="icon"
                    variant="outline"
                    onClick={() => copyReferral(referralLink, "Link")}
                  >
                    <Copy className="w-4 h-4" />
                  </Button>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <DollarSign className="w-5 h-5 text-primary" /> Conversion
            </CardTitle>
          </CardHeader>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <CardTitle className="flex items-center gap-2">
              <Trophy className="w-5 h-5 text-primary" /> Live Events
            </CardTitle>
            <Button variant="ghost" size="sm" onClick={() => goTo("events")}>
              View all
            </Button>
          </CardHeader>
          <CardContent className="space-y-3">
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <CardTitle className="flex items-center gap-2">
              <Crown className="w-5 h-5 text-primary" /> Open Rooms
            </CardTitle>
            <Button variant="ghost" size="sm" onClick={() => goTo("rooms")}>
              View all
            </Button>
          </CardHeader>
          <CardContent className="space-y-3">
            {openRooms.slice(0, 3).map((r) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground capitalize">
                    Level {r.level} · +{r.rewardPoints} pts
                  </p>
                </div>
                <div className="text-right flex-shrink-0">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <CardTitle className="flex items-center gap-2">
              <Brain className="w-5 h-5 text-primary" /> Quick Quiz
            </CardTitle>
            <Button variant="ghost" size="sm" onClick={() => goTo("quizzes")}>
              Open
            </Button>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between gap-3 rounded-md border p-3">
              <div>
                <p className="font-medium text-sm">
                  Take a quiz and earn bonus points
                </p>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 max-h-64 overflow-y-auto">
            {myNotifs.slice(0, 5).map((n) => (
              <div
                key={n.id}
                className={`p-2 rounded-md text-sm ${n.read ? "" : "bg-primary/5"}`}
              >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <Button variant="ghost" size="sm" onClick={() => goTo("tasks")}>
              View all
            </Button>
          </CardHeader>
            ...
            {recentTasks.map((t) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t.completed}/{t.availability} completed
                  </p>
                </div>
                <Badge
                  variant="outline"
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Clock className="w-5 h-5 text-primary" /> Withdrawal Status
            </CardTitle>
          </CardHeader>
          <CardContent>
