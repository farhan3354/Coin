...
export function StatCard({ title, value, subtitle, icon: Icon, accent = "bg-primary/10 text-primary", className, delay = 0 }: StatCardProps) {
  return (
    <motion.div
      ...
      className="min-w-0"
    >
      <Card className={cn("overflow-hidden transition-shadow hover:shadow-md min-w-0", className)}>
        <CardContent className="p-3 sm:p-5">
          <div className="flex items-start justify-between gap-2 min-w-0">
            ...
            <motion.div
              className={cn("grid place-items-center w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex-shrink-0", accent)}
